...
const STATUSES = ['fired', 'applied', 'shortlisted', 'selected', 'rejected'];
...
export default function PlacementApplications() {
  ...
  const setStatus = async (a, status) => { try { await placementApi.setApplicationStatus(a.id, { status }); load(); } catch (e) { setToast({ severity: 'error', text: e.message }); } };
      ...
      {openingId && (loading ? <Card><Skeleton h={16} w="40%" /><div style={{ height: 8 }} /><Skeleton h={12} w="70%" /></Card>
        : rows.length === 0 ? <Card><EmptyState icon="📋" title="No candidates yet" text="Fire this opening to eligible students, or wait for applications." /></Card> : (
          <Card pad={0} style={{ overflow: 'hidden' }}>
            <Table size="small">
              <TableHead><TableRow sx={{ background: '#fafbfc', '& th': { color: '#64748b', fontWeight: 700, fontSize: 12 } }}><TableCell>Student</TableCell><TableCell align="center">Status</TableCell><TableCell align="right">Move to</TableCell></TableRow></TableHead>
              <TableBody>{rows.map((a) => (
                <TableRow key={a.id} hover>
                  <TableCell>{a.name}<div style={{ fontSize: 11, color: '#94a3b8' }}>{a.email}</div></TableCell>
                  <TableCell align="center"><Badge tone={tone(a.status)}>{a.status}</Badge></TableCell>
